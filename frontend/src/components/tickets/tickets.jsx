...
/* ---------- Componente Tickets: lógica previa al return ---------- */
export default function Tickets() {
    ...
    const stats = useMemo(() => {
        ...
        return { open, inProgress, pending, resolved, resolvedToday, avgResponseMin, urgent };
    }, [tickets]);
    ...
    // handleSubmit, handleEdit, handleDelete, handleChange remain as in tu lógica original
    ...
    const handleDelete = (id) => {
        if (window.confirm("¿Seguro que deseas borrar este ticket?")) {
        fetch(`http://localhost:3000/routes/tickets/${id}`, { method: "DELETE" })
            .then(() => setTickets((prev) => prev.filter((t) => t.ticket_id !== id)))
            .catch((err) => console.error(err));
        }
    };

    // Fin de la parte lógica; a continuación va el return JSX
    ...
    <div className="p-4 sm:p-6 bg-[#0f1113] min-h-screen text-gray-200">
        ...
        <div className="hidden md:block bg-[#0b0c0d]/60 border border-[#222] rounded-xl overflow-hidden">
            ...
            <table className="min-w-full divide-y divide-[#1f1f1f]">
                ...
                {filteredTickets.map((t) => (
                    <tr key={t.ticket_id} className="hover:bg-white/2 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <div className="relative w-full max-w-3xl md:max-w-4xl lg:max-w-5xl bg-gradient-to-br from-gray-800 to-gray-900 p-6 rounded-xl shadow-2xl border border-[#222] z-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <input name="cliente_nombre" value={newTicket.cliente_nombre} onChange={handleChange} placeholder="Nombre cliente" className="bg-[#0b0c0d] border border-[#222] rounded-md px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-orange-500" />
                <input name="cliente_email" value={newTicket.cliente_email} onChange={handleChange} placeholder="Email cliente" className="bg-[#0b0c0d] border border-[#222] rounded-md px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-orange-500" />
                </div>

                <textarea name="descripcion_falla" value={newTicket.descripcion_falla} onChange={handleChange} placeholder="Descripción de la falla" rows="4" className="w-full bg-[#0b0c0d] border border-[#222] rounded-md px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-orange-500" />

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <select name="prioridad" value={newTicket.prioridad} onChange={handleChange} className="bg-[#0b0c0d] border border-[#222] rounded-md px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-orange-500">
                ...
                <select name="estado_ticket" value={newTicket.estado_ticket} onChange={handleChange} className="bg-[#0b0c0d] border border-[#222] rounded-md px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-orange-500">
                    <option>Open</option>
                ...
                <input name="equipo_id" value={newTicket.equipo_id} onChange={handleChange} placeholder="Equipo ID" className="bg-[#0b0c0d] border border-[#222] rounded-md px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-orange-500" />
                </div>

                <div className="flex items-center justify-end gap-3">
                <button type="button" onClick={closeModal} className="px-4 py-2 rounded-md bg-white/5 text-gray-200">Cancelar</button>
                <button type="submit" className="px-4 py-2 rounded-md bg-orange-500 text-white">{editingTicket ? "Guardar cambios" : "Crear ticket"}</button>
                </div>
            </form>
            </div>
        </div>
